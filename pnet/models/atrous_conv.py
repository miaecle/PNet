...
class AtrousConvContactMap(ConvNetContactMapBase):
  ...
  def Res1DAtrousModule_b(self, n_input, in_layer, rate=2):
  ...
  def Res2DAtrousModule_b(self, n_input, in_layer, rate=2, res_flag_2D=None):
  ...
  def Conv1DModule(self, n_input, in_layer):
    ...
    # n_input = 128
    n_input, in_layer = self.Res1DModule_a(n_input, in_layer)
  ...
  def atrous_structure(self, n_input, in_layer):
    ...
    self.shortcut_shapes = [ToShape(n_input, self.batch_size, in_layers=[self.n_residues])]
    ...
    for i in range(n_pool_layers):
      ...
      self.shortcut_shapes.append(ShapePool(n_filter=n_input, in_layers=[self.shortcut_shapes[-1]]))
